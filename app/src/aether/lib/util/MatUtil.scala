...
object MatUtil {
  ...
  def orthoTransform(x1: Float, x2: Float, y1: Float, y2: Float): Transform2F = {
    val scaleX = 2 / (x2 - x1)
    val scaleY = 2 / (y2 - y1)
    Transform2F.identity.translate(-MathF.sign(scaleX), -MathF.sign(scaleY)).scale(scaleX, scaleY)
  }
  ...
  def perspective(fovyInDegrees: Float, aspectRatio: Float, znear: Float, zfar: Float): Mat4F = {
    val ymax = znear * MathF.tan(fovyInDegrees * MathF.Pi / 360.0f)
  ...
  def perspectiveBalanced(fovDegrees: Float, sizeX: Float, sizeY: Float, znear: Float, zfar: Float): Mat4F = {
    val n = znear * MathF.tan(fovDegrees * MathF.Pi / 360.0f)
    val ratioSqrt = MathF.sqrt(sizeX / sizeY)
